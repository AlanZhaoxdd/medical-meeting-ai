...
> 用途：这是整份面经问答库的**人读版**，供快速阅读、背诵与追问演练。配套的 KB 入库版为 `interview-qa-kb.md`，内容一致但已做切分预处理（每题独立成块），可直接上传知识库测试 RAG。
>
> 内容：8 组 65 题。素材来自牛客网 2026 年 AI 应用开发岗位面经（大模型应用开发 5 年经验、校招 AI 岗「从基础到死亡追问」、凌脉/蚂蚁/安软/淘天 AI 应用开发实习与一面/二面、AI 岗八股与普通开发岗的区别等），每题按「一句话记忆点 + 展开」组织，展开尽量结合本项目代码。
>
> 使用建议：先背每组的一句话记忆点，再读展开；被追问细节时按展开分层作答。标记「了解即可」的题目项目未直接实现，回答时须诚实说明"了解原理但未实践"。

## 目录

- [一、项目背景与自我介绍](#一项目背景与自我介绍)（1–5）
- [二、通用/架构](#二通用架构)（6–11）
- [三、RAG 深度（面经高频）](#三rag-深度面经高频)（12–27）
- [四、解析与入库](#四解析与入库)（28–36）
- [五、向量与检索](#五向量与检索)（37–41）
- [六、LLM 应用与 Agent](#六llm-应用与-agent)（42–48）
- [七、工程与可靠性](#七工程与可靠性)（49–60）
- [八、基础 AI 八股（了解即可/可能被追问）](#八基础-ai-八股了解即可可能被追问)（61–65）
- [附录：如何用 KB 版测试 RAG](#附录如何用-kb-版测试-rag)
...
展开：① 建 bad case 台账：记录 query、期望、实际输出、检索四类分数、命中的 chunk、模型输出全文，保证可复现；② 归类定位：检索失败 / 生成失败 / 数据缺失 / 评测问题，按"检索 vs 生成"的分层归因；③ 修复后回归：同一黄金集重跑 + 人工抽检，确认没有引入新问题。面试话术：**"bad case 不是玄学，是可复现、可归类、可回归的流程。"**
...
1. **导入**：把 `interview-qa-kb.md` 上传到知识库。它已按本项目解析器预处理：UTF-8 编码、`#` 层级标题维护 heading_path、每题是 `### 问题` 标题 + 单行答案段落、无任何 markdown 装饰符号（粗体/引用块/表格），切块后**每题问答独立成一个 chunk**。
2. **发布**：等切块与向量化完成后发布（检索默认只返回 PUBLISHED 内容；草稿检索需 editor 角色并写审计日志）。
3. **检索验证**：在检索工作台直接输入任意一题的题目文本（例如"RAG 与微调的区别是什么？"），预期第一个结果就是该题对应的答案块；可用 `eval_retrieval.py` 的思路，把 65 题题目作为 golden query、对应 chunk 作为 expected，跑 Recall@k/MRR。
4. **切块观察**：在纪要编辑页看 chunk 边界，确认每题 1 块、答案未被截断、标题进入 heading_path。
5. **区分度测试**：25.3 组内有多道语义相近的题（如"如何防 LLM 幻觉"与"RAG 还会幻觉吗"、"RRF 的 k 为什么取 60"与"多路召回是什么"），检索时应各自命中自己的答案块，可用来验证混合检索 + rerank 的区分能力。
